...
import { 
  getLatestQuotes, 
  getSolanaTokens as getCMCSolanaTokens,
...
} from "./api/quicknode.js";
import { z } from "zod";
import { 
  insertArbitrageOpportunitySchema, 
  insertTokenSchema, 
  insertWalletTrackingSchema 
...
async function setupEventListeners(broadcast: (data: any) => void) {
...
function setupApiRoutes(app: Express, broadcast: (data: any) => void) {
  // API status endpoint
  ...
  app.get('/api/arbitrage/opportunities', async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/arbitrage/execute', async (req: Request, res: Response) => {
    try {
      const { id } = req.body;
  ...
  app.get('/api/arbitrage/settings', async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/arbitrage/settings', async (req: Request, res: Response) => {
    try {
  ...
  // Get token prices
  app.get('/api/tokens/prices', async (req: Request, res: Response) => {
    try {
      ...
      const priceData = await getPrices(symbolsArray.map(symbol => symbol) as string[]) as PriceData;
  ...
  // Wallet tracking
  app.post('/api/wallet/track', async (req: Request, res: Response) => {
    try {
      // Validate input
  ...
  // Get tracked wallets
  app.get('/api/wallet/tracking', async (req: Request, res: Response) => {
    try {
  ...
  app.patch('/api/wallet/tracking/:id', async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/jupiter/quote', async (req: Request, res: Response) => {
    try {
      ...
      const quote = await getQuote(
        ...
        amount as string,
        slippageBps ? parseInt(slippageBps as string) : undefined
      );
