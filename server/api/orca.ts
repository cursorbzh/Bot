...
// Cache pour éviter des appels API redondants
interface PoolInfo {
  id: string;
  tokenA: string;
  tokenB: string;
 ...
 */
async function fetchOrcaPools(): Promise<PoolInfo[]> {
  ...
  try {
    ...
    if (response.status === 200 && response.data && Array.isArray(response.data.pools)) {
      poolsCache = response.data.pools.map((pool: any) => ({
        id: pool.address,
 ...
 */
async function findOrcaPool(inputMint: string, outputMint: string): Promise<PoolInfo | null> {
  try {
    ...
    const inversePool = pools.find(pool => 
