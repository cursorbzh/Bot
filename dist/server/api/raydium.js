 ...
 */
async function findLiquidityPool(inputMint, outputMint) {
    const pools = await fetchLiquidityPools();
    // Recherche du pool pour la paire spécifiée (dans les deux sens)
    const pool = pools.find(pool => (pool.baseMint === inputMint && pool.quoteMint === outputMint) ||
    ...
    if (!pool.baseReserve || !pool.quoteReserve ||
        ...
        console.log(`Pool trouvé mais données invalides pour ${inputMint}/${outputMint}:`, JSON.stringify({ baseReserve: pool.baseReserve, quoteReserve: pool.quoteReserve }));
        return null;
    }
    ...
    if (isNaN(baseReserve) || isNaN(quoteReserve) || baseReserve <= 0 || quoteReserve <= 0) {
        console.log(`Pool trouvé mais réserves invalides pour ${inputMint}/${outputMint}:`, { baseReserve, quoteReserve });
        return null;
    }
 ...
 */
function calculateEstimatedPrice(pool, inputMint, amountIn) {
 ...
 */
export async function getRaydiumQuote(inputMint, outputMint, amountIn, slippageBps = 50) {
