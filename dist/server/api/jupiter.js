...
function getQuoteFromCache(inputMint, outputMint, amount) {
...
function addQuoteToCache(inputMint, outputMint, amount, quote, source) {
    const key = getCacheKey(inputMint, outputMint, amount);
    quoteCache[key] = {
        quote,
        source,
 ...
 */
export async function getQuote(inputMint, outputMint, amount, slippageBps = 50, onlyDirectRoutes = false) {
    try {
 ...
 */
export async function getPrice(inputMint, outputMint) {
    try {
 ...
 */
export async function prepareSwapTransaction(quoteResponse, userPublicKey) {
    try {
        // Use rate limited fetch
 ...
 */
export async function scanForArbitrageOpportunities(tokenPairs) {
    const results = [];
    ...
    async function getQuoteWithCache(inputMint, outputMint, amount) {
    ...
    for (const pair of tokenPairs) {
        try {
            ...
            const forward = await testArbitragePath(pair.inputMint, pair.outputMint, forwardAmount, getQuoteWithCache);
            if (forward) {
                results.push(forward);
            }
            // Tester le sens inverse
            const backward = await testArbitragePath(pair.outputMint, pair.inputMint, forwardAmount, getQuoteWithCache);
            if (backward) {
...
async function testArbitragePath(startMint, endMint, amount, getQuoteWithCache) {
    // Quote aller
    ...
    try {
        ...
        if (finalAmount >= initialAmount * BigInt(99) / BigInt(100)) {
            ...
            const result = {
                ...
                routes: {
                    forward: Array.isArray(forward.quote.routePlan) ? forward.quote.routePlan : [],
                    backward: Array.isArray(backward.quote.routePlan) ? backward.quote.routePlan : []
                },
                sources: {
