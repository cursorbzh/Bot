...
const activeArbitrageScans = new Map();
(async () => {
    ...
    const cleanupConnection = (clientId) => {
        ...
        ws.on('message', (message) => {
            try {
                ...
                if (data.type === 'ping') {
                    ...
                    storage.getArbitrageSettings().then((settings) => {
                        ...
                        storage.getAllTokens().then(async (tokens) => {
                            ...
                            const baseTokens = [];
                            if (solToken)
                                baseTokens.push(solToken);
                            else
                                baseTokens.push({ id: 0, symbol: 'SOL', name: 'Solana', address: solAddress });
                            if (usdcToken)
                                baseTokens.push(usdcToken);
                            else
                                baseTokens.push({ id: 0, symbol: 'USDC', name: 'USD Coin', address: usdcAddress });
                            if (usdtToken)
                                baseTokens.push(usdtToken);
                            else
                                baseTokens.push({ id: 0, symbol: 'USDT', name: 'Tether', address: usdtAddress });
                            ...
                            try {
                                ...
                                if (!opportunities || opportunities.length === 0) {
                                    ...
                                    const filteredOpportunities = opportunities.filter(opp => opp.profitPercentage >= minSpreadPercentage);
                                    ...
                                    try {
                                        ...
                                        const enrichedOpportunities = await Promise.all(dbOpportunities.map(async (opp) => {
                                            try {
                            ...
                            const intervalId = setInterval(async () => {
                                try {
                                    ...
                                    // Filtrer les opportunités selon les paramètres
                                    const minSpreadPercentage = settings.minSpreadPercentage > 0 ? 0.01 : settings.minSpreadPercentage;
                                    const filteredOpportunities = allOpportunities.filter(opp => opp.profitPercentage >= minSpreadPercentage);
                                    ...
                                    for (const opp of filteredOpportunities) {
                                        try {
                                            ...
                                            if (tokenInput && tokenOutput) {
                                                ...
                                                const existingOpp = existingOpps.find(ex => ex.tokenId === tokenInput.id);
                                                if (existingOpp) {
                                                    // Mettre à jour l'opportunité existante
                                                    await storage.updateArbitrageOpportunity(existingOpp.id, existingOpp.executed || false // Garder le même statut d'exécution
                                                    );
                                                }
                                                else {
                                    ...
                                    try {
                                        ...
                                        // Enrichir les opportunités avec les données complètes des tokens
                                        const enrichedOpportunities = await Promise.all(updatedOpportunities.map(async (opp) => {
                                            try {
                                                const token = await storage.getToken(opp.tokenId);
                                                return {
                                                    ...opp,
